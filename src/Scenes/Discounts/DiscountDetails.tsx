...
function DiscountDetails() {
	const { id } = useParams();
	const { data: discount } = useFetchData(`/discounts/${id}`);
	...
	return (
		<div className="w-full">
			...
			{discount && (
				<form onSubmit={handleSubmit(updateDiscount)} className="p-4 bg-white ">
					<div className="w-2/3 mx-auto ">
						...
						<div className="flex w-full ">
							...
							<div className="w-1/2">
								...
								<Controller
									name="endAt"
									control={control}
									defaultValue={new Date(discount.endAt)}
									render={({ field }) => (
										<DatePicker
											placeholderText="select received date"
											onChange={(date: Date) => field.onChange(date)}
						...
						<div className="flex w-full gap-3 ">
							<div className="w-1/2">
								...
								<input
									type="number"
									defaultValue={discount.numberOfItems}
							...
							<div className="w-1/2">
								...
								<div className="flex items-center gap-3">
									<input
										type="number"
										defaultValue={discount.costOfOrder}
						...
						<p className="text-sm font-bold text-blue-900">
							{discount.type === "ALL_PRODUCTS" && "All products"}
						</p>
						{discount.products.length !== 0 && (
							<div>
								...
								<div className="grid grid-cols-4 gap-3 mt-4">
									{discount.products.map((el: product) => (
						...
						{discount.categories.length !== 0 && (
							<div>
								...
								<div className="grid grid-cols-4 gap-3 mt-4">
									{discount.categories.map((el: category) => (
										<p className="px-3 text-black bg-slate-400 ">{el.name}</p>
									))}
								</div>
							</div>
